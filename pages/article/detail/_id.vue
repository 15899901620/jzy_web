      ...
      <div class="whitebg " style="width: 80%;">
        ...
        <div class="new_other">
          <span v-if="lastPage_id">【上一篇】
             <nuxt-link :to="{name: 'article-detail-id', params:{ id: lastPage_id }}">{{ lastPage }}</nuxt-link>
          </span>
          <span v-if="nextPage_id">【下一篇】
             <nuxt-link :to="{name: 'article-detail-id', params:{ id: nextPage_id }}">{{ nextPage }}</nuxt-link>
          </span>
        </div>
      </div>
