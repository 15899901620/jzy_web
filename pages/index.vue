...
export default {
  async asyncData ({app}) {
    ...
    .then((res) => {
      return {datalist: res.data.items}
    })
  },
  fetch({ store }) {
    return Promise.all([
      ...
      store.dispatch('system/getBannerInfo',{positionId: 2}),
      store.dispatch('article/getHomeNotice',{current_page: 1, page_size: 8}),
      store.dispatch('article/getHomeArticle',{current_page: 1, page_size: 8}),
      store.dispatch('article/getHotArticle',{current_page: 1, page_size: 4})
    ])
  },
  components: {
    Header,
  ...
  data() {
    return {
      moreData:{title:'更多...', url:'/article/2'},
      heightNum: 320
    }
  },
  methods: {
