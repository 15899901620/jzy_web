      ...
      <div class="memberInfor ml20  whitebg bdccc  mt20">
        ...
        <div class="" style="width: 95%; margin: 0 auto;">
          <div class="order_operate">
            <div class="dflex">
              <input type="text" placeholder="合约编号、商品名称搜索" ref="searchval" class="orderInput"
                     v-model="formSearch.sku_name"/>
          ...
          <template v-if="monthinfo.length > 0">
            <table v-for="(item, index) in monthinfo" :key="index" class="listT mt10" border="" cellspacing=""
                   cellpadding="">
              <tbody>
              <tr class="detailTable">
                <td style="width: 5%;">{{item.year}}</td>
                <td style="width: 5%;">{{item.canMonth}}</td>
                <td style="width: 15%;"><span @click='planNo(item)' style="cursor: pointer; color: #007de4">{{item.planNo}}</span>
                </td>
                <td style="width: 20%;">{{item.skuName}}</td>
                <td style="width: 10%;">
                  {{item.estimateNum}}吨
                </td>
                ...
                <td style="width: 10%;" class="operate">
                  <Tooltip content="添加月计划" placement="left">
                    <Button title="添加月计划" icon="md-add" @click='addmonth(item)' size="small"></Button>
                  </Tooltip>
                </td>
              </tr>
              </tbody>
          ...
          <pages :total="monthTotal" :show-total="showTotal" :value="current_page" :pageSize="6"
                 style="margin:20px 0;"></pages>
          <yeardetail :isshow="detailmodal" :datalist="rowPlanData" @unChange="undetail"
                      @onChange="editOnSuccess"></yeardetail>
...
export default {
	...
	fetch({store, query}) {
		return Promise.all([
			...
			//获取会员合约列表
			store.dispatch('special/monthaddPlanList', {
