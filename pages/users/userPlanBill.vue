  ...
  <div class="clearfix graybg">
    <div class="w1200 dflex " style="margin-bottom: 40px">
      ...
      <div class="memberInfor ml20  whitebg bdccc  mt20" >
        ...
        <div class="bb1 pb10" style="width: 95%; margin: 0 auto;" v-if="$store.state.spot.spotPlanList.length > 0">
        ...
        <div style="width: 95%; margin: 0 auto;" class="bb1 pb10" v-if="$store.state.bidders.planList.length > 0">
          <div class="order_operate">
          ...
          <table v-for="(item, index) in $store.state.bidders.planList" :key="index" class="listT mt10" border=""
                 cellspacing="" cellpadding="">
            <tbody>
            <tr class="Ttitle graybg">
              <td colspan="8">
                ...
                <span class="fr mr15"
                      v-if="item.status != 3 && item.availableNum > 0 && (item.closeApplyStatus == 1 || item.closeApplyStatus == 4)">
                      <span class="red">转单倒计时：</span>
                      <span class="red"><TimeDown :endTime="item.lastOrderedDate" hoursShow endMsg="已失效"
                                                  :onTimeOver="$utils.reload"></TimeDown></span>
                  </span>
              </td>
            </tr>
            <tr class="detailTable">
              ...
              <td style="width: 15%;" class="operate">
                ...
                <div v-else-if="item.availableNum > 0">
                  <template v-if="item.closeApplyStatus == 1">
                    <a class="Paybtn CarCurr" style="padding: 3px 6px" :href="`/auction/change/${item.id}`">转单</a>
                  </template>
        ...
        <div style="width: 95%; margin: 0 auto;" v-if="$store.state.member.orderList.length > 0">
          <div class="order_operate">
	...
	export default {
		...
		fetch({store, query}) {
			return Promise.all([
				...
				store.dispatch('bidders/getPlanList', {
					current_page: query.page || 1,
					status: 1,
					page_size: 5,
					planNo: query.plan_no ? query.plan_no : ''
				})
			])
		},
		data() {
